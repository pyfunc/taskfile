...
class TestRegressionEnvFlag:
    ...
    def test_remote_respects_env_flag(self, tmp_path, monkeypatch):
        ...
        # staging should NOT appear in the Remote Server Diagnostics section
        # (it may appear in the regular verbose SSH checks, but not in remote diag)
        remote_section = result.output.split("Remote Server Diagnostics")
        if len(remote_section) > 1:
            assert "staging.example.com" not in remote_section[1].split("Zależności")[0] if "Zależności" in remote_section[1] else remote_section[1]
