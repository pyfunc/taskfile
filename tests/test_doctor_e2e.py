...
# ═══════════════════════════════════════════════════════
# Registry access diagnostics
# ═══════════════════════════════════════════════════════


class TestExtractRegistryHost:
    """Unit tests for _extract_registry_host helper."""

    def test_remote_registry(self):
        from taskfile.diagnostics.checks import _extract_registry_host
        assert _extract_registry_host("ghcr.io/user/app:latest") == "ghcr.io"
        assert _extract_registry_host("docker.io/library/nginx") == "docker.io"
        assert _extract_registry_host("registry.example.com:5000/myapp:v1") == "registry.example.com:5000"

    def test_local_images(self):
        from taskfile.diagnostics.checks import _extract_registry_host
        assert _extract_registry_host("myapp:latest") is None
        assert _extract_registry_host("nginx") is None
        assert _extract_registry_host("localhost/myapp:latest") is None
        assert _extract_registry_host("127.0.0.1/myapp") is None
        assert _extract_registry_host("localhost:5000/myapp") is None

    def test_user_prefix_no_dot(self):
        """user/image is not a registry (no dot in first segment)."""
        from taskfile.diagnostics.checks import _extract_registry_host
        assert _extract_registry_host("myuser/myapp:latest") is None

    def test_with_digest(self):
        from taskfile.diagnostics.checks import _extract_registry_host
        assert _extract_registry_host("ghcr.io/user/app@sha256:abc123") == "ghcr.io"


class TestCheckRegistryAccess:
    """E2E tests for check_registry_access diagnostic."""

    def test_no_images_no_issues(self, tmp_path, monkeypatch):
        """No IMAGE variables → no registry issues."""
        from taskfile.diagnostics.checks import check_registry_access
        ...
        issues = check_registry_access(config)
        assert issues == []

    def test_local_images_no_issues(self, tmp_path, monkeypatch):
        """Images without registry prefix → no issues."""
        from taskfile.diagnostics.checks import check_registry_access
        from taskfile.parser import load_taskfile
        monkeypatch.chdir(tmp_path)
        tf = tmp_path / "Taskfile.yml"
        tf.write_text(yaml.dump({
            ...
            "name": "local-images",
            "variables": {
                "IMAGE_WEB": "myapp-web:latest",
                "IMAGE_API": "localhost/myapp-api:latest",
            },
            "tasks": {"t": {"cmds": ["echo"]}},
        }))
        config = load_taskfile(tf)
    ...
    def test_unreachable_registry_warns(self, tmp_path, monkeypatch):
        """Unreachable registry → warning with taskfile push suggestion."""
        from taskfile.diagnostics.checks import check_registry_access
        from taskfile.parser import load_taskfile
        monkeypatch.chdir(tmp_path)
        tf = tmp_path / "Taskfile.yml"
        tf.write_text(yaml.dump({
            ...
            "name": "unreachable",
            "variables": {
                "IMAGE_WEB": "registry.invalid.test/myapp:latest",
            },
            "tasks": {"t": {"cmds": ["echo"]}},
        }))
        # Mock registry check to simulate unreachable
        monkeypatch.setattr(
            "taskfile.diagnostics.checks._check_registry_reachable",
            lambda host, timeout=3.0: False,
        )
        config = load_taskfile(tf)
        issues = check_registry_access(config)
        assert len(issues) == 1
        assert "registry.invalid.test" in issues[0].message
        assert "not reachable" in issues[0].message
        assert "taskfile push" in issues[0].fix_description

    def test_env_specific_images(self, tmp_path, monkeypatch):
        """IMAGE variables in environment-specific config are also checked."""
        from taskfile.diagnostics.checks import check_registry_access
        from taskfile.parser import load_taskfile
        monkeypatch.chdir(tmp_path)
        tf = tmp_path / "Taskfile.yml"
        tf.write_text(yaml.dump({
            ...
            "environments": {
                "prod": {
                    "variables": {
                        "IMAGE_WEB": "registry.invalid.test/prod-web:latest",
                    },
                },
            },
            "tasks": {"t": {"cmds": ["echo"]}},
        ...
        issues = check_registry_access(config)
        assert len(issues) >= 1
        assert any("registry.invalid.test" in i.message for i in issues)

    def test_doctor_cli_shows_registry_warning(self, tmp_path, monkeypatch):
        """taskfile doctor shows registry warning in CLI output."""
        from taskfile.cli.interactive.wizards import main
        monkeypatch.chdir(tmp_path)
        tf = tmp_path / "Taskfile.yml"
        tf.write_text(yaml.dump({
        ...
        runner = CliRunner()
        result = runner.invoke(main, ["doctor"])
        assert "registry.invalid.test" in result.output
        assert "not reachable" in result.output


# ═══════════════════════════════════════════════════════
# Runtime error classification — localhost pull pattern
# ═══════════════════════════════════════════════════════


class TestClassifyLocalhostPull:
    """E2E tests for classify_runtime_error detecting localhost/ pull failures."""

    def test_localhost_pull_connection_refused(self):
        """podman pull localhost/ + connection refused → specific diagnosis."""
        from taskfile.diagnostics.llm_repair import classify_runtime_error
        stderr = (
            'Trying to pull localhost/taskfile-example-web:latest...\n'
            'Error: initializing source docker://localhost/taskfile-example-web:latest: '
            'pinging container registry localhost: Get "https://localhost/v2/": '
            'dial tcp [::1]:443: connect: connection refused'
        )
        issue = classify_runtime_error(125, stderr, "podman pull localhost/taskfile-example-web:latest")
        assert issue.category == IssueCategory.CONFIG_ERROR
        assert "no local registry" in issue.message
        assert "taskfile push" in issue.fix_description
        assert "taskfile-example-web" in issue.message

    def test_localhost_pull_extracts_image_name(self):
        """Image name is extracted from stderr."""
        from taskfile.diagnostics.llm_repair import classify_runtime_error
        stderr = (
            'Trying to pull localhost/myapp-landing:v2.0...\n'
            'Error: connection refused'
        )
        issue = classify_runtime_error(125, stderr, "@remote podman pull ${IMAGE}")
        assert "myapp-landing:v2.0" in issue.message

    def test_non_localhost_connection_refused(self):
        """Non-localhost connection refused → generic EXTERNAL_ERROR."""
        from taskfile.diagnostics.llm_repair import classify_runtime_error
        stderr = "Error: connection refused to ghcr.io"
        issue = classify_runtime_error(1, stderr, "curl https://ghcr.io")
        assert issue.category == IssueCategory.EXTERNAL_ERROR

    def test_localhost_no_pull_is_generic(self):
        """localhost + connection refused without pull → generic error."""
        from taskfile.diagnostics.llm_repair import classify_runtime_error
        stderr = "Error: connection refused to localhost:8080"
        issue = classify_runtime_error(1, stderr, "curl http://localhost:8080")
        assert issue.category == IssueCategory.EXTERNAL_ERROR


class TestExtractImageName:
    """Unit tests for _extract_image_name helper."""

    def test_from_trying_to_pull(self):
        from taskfile.diagnostics.llm_repair import _extract_image_name
        stderr = "Trying to pull localhost/myapp:latest..."
        assert _extract_image_name("", stderr) == "localhost/myapp:latest"

    def test_from_cmd(self):
        from taskfile.diagnostics.llm_repair import _extract_image_name
        assert _extract_image_name("podman pull ghcr.io/user/app:v1", "") == "ghcr.io/user/app:v1"
        assert _extract_image_name("docker pull myapp:latest", "") == "myapp:latest"

    def test_fallback(self):
        from taskfile.diagnostics.llm_repair import _extract_image_name
        assert _extract_image_name("echo hello", "some error") == "unknown-image"
